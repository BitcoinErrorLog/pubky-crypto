...
//! ```toml
//! [dependencies]
//! pubky-crypto = { version = "0.1", features = ["secure-mem"] }
//! ```
//!
//! ## Behavior
//!
...
pub fn mlock_slice(_data: &mut [u8]) -> Result<bool, CryptoError> {
...
#[cfg(feature = "secure-mem")]
pub fn munlock_slice(data: &mut [u8]) -> Result<(), CryptoError> {
    use region::unlock;
    ...
    match unlock(data.as_ptr(), data.len()) {
        Ok(()) => Ok(()),
        Err(_) => {
            // Best-effort: continue without unlock
            Ok(())
        }
    }
}
...
///
/// ```rust,ignore
/// use pubky_crypto::secure_mem::LockedBytes;
///
/// let mut key = [0u8; 32];
