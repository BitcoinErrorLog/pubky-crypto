...
fn encode_cert_body(input: &AppCertInput) -> Result<Vec<u8>, CryptoError> {
    // Validate inputs
...
pub fn verify_app_cert(
    ...
    cert_body: &[u8],
    sig: &[u8; 64],
) -> Result<[u8; CERT_ID_LEN], CryptoError> {
    let verifying_key = VerifyingKey::from_bytes(issuer_peerid)
        .map_err(|_| CryptoError::Other("Invalid issuer public key".to_string()))?;
    ...
    verifying_key
        ...
        .map_err(|_| CryptoError::Other("AppCert signature verification failed".to_string()))?;
...
pub fn verify_typed_content(
    ...
    let verifying_key = VerifyingKey::from_bytes(app_ed25519_pub)
        .map_err(|_| CryptoError::Other("Invalid AppKey public key".to_string()))?;
    let signature = ed25519_dalek::Signature::from_bytes(sig);
    
    verifying_key
        .verify_strict(&sign_input, &signature)
        .map_err(|_| CryptoError::Other("Typed content signature verification failed".to_string()))
}
...
impl KeyBinding {
    ...
    pub fn decode(data: &[u8]) -> Result<Self, CryptoError> {
        ...
        if data.is_empty() {
            return Err(CryptoError::Decryption("Empty KeyBinding data".into()));
        }
        ...
        if map_len > 16 {
            return Err(CryptoError::Decryption("KeyBinding: too many keys".into()));
        }
        ...
        for _ in 0..map_len {
            if pos >= data.len() {
                return Err(CryptoError::Decryption("KeyBinding: truncated".into()));
            }
...
fn cbor_decode_bytes(data: &[u8]) -> Result<(Vec<u8>, usize), CryptoError> {
    if data.is_empty() {
        return Err(CryptoError::Decryption("cbor_decode_bytes: empty input".into()));
    }
    let first = data[0];
    let major = first >> 5;
    if major != 2 {
    ...
    let (byte_len, header_size): (usize, usize) = if additional < 24 {
        ...
        if data.len() < 2 {
            return Err(CryptoError::Decryption("cbor_decode_bytes: truncated length".into()));
        }
        (data[1] as usize, 2)
    ...
    if data.len() < total {
        return Err(CryptoError::Decryption("cbor_decode_bytes: truncated data".into()));
    }
    Ok((data[header_size..total].to_vec(), total))
}

/// Decode CBOR array header, returning (array_length, consumed_bytes).
fn cbor_decode_array_header(data: &[u8]) -> Result<(usize, usize), CryptoError> {
    if data.is_empty() {
        return Err(CryptoError::Decryption("cbor_decode_array_header: empty input".into()));
    }
    let first = data[0];
    let major = first >> 5;
    if major != 4 {
    ...
    if additional < 24 {
        ...
        Err(CryptoError::Decryption(format!(
...
fn decode_inbox_keys_array(data: &[u8]) -> Result<(Vec<InboxKeyEntry>, usize), CryptoError> {
    let (arr_len, mut pos) = cbor_decode_array_header(data)?;
    let mut entries = Vec::with_capacity(arr_len);

    for _ in 0..arr_len {
        if pos >= data.len() {
            return Err(CryptoError::Decryption("decode_inbox_keys_array: truncated".into()));
        }
        let first = data[pos];
        if first != 0x82 {
            return Err(CryptoError::Decryption(format!(
...
fn decode_transport_keys_array(data: &[u8]) -> Result<(Vec<TransportKeyEntry>, usize), CryptoError> {
    let (arr_len, mut pos) = cbor_decode_array_header(data)?;
    let mut entries = Vec::with_capacity(arr_len);
...
fn decode_app_keys_array(data: &[u8]) -> Result<(Vec<AppKeyEntry>, usize), CryptoError> {
    let (arr_len, mut pos) = cbor_decode_array_header(data)?;
    let mut entries = Vec::with_capacity(arr_len);

    for _ in 0..arr_len {
        if pos >= data.len() {
            return Err(CryptoError::Decryption("decode_app_keys_array: truncated".into()));
        }
        let first = data[pos];
        if first != 0x82 {
            return Err(CryptoError::Decryption(format!(
...
mod tests {
    ...
    fn test_keybinding_decode_wrong_inbox_kid_length() {
        let mut data = vec![
            ...
            0x81, // array(1)
            0x82, // array(2)
            0x48, // bytes(8) — wrong length, should be 16
        ];
        data.extend([0u8; 8]);
        data.push(0x58); // bytes(32)
        data.push(32);
        data.extend([0u8; 32]);
