...
impl<'a> CborReader<'a> {
    ...
    #[allow(dead_code)]
    fn peek(&self) -> Result<u8, CryptoError> {
        if self.pos >= self.data.len() {
            return Err(CryptoError::Decryption("CBOR: unexpected end of data".into()));
        }
    ...
    fn read_argument(&mut self, additional: u8) -> Result<u64, CryptoError> {
        if additional < 24 {
            ...
            if self.remaining() < 2 {
                return Err(CryptoError::Decryption("CBOR: truncated u16".into()));
            }
            ...
            if self.remaining() < 4 {
                return Err(CryptoError::Decryption("CBOR: truncated u32".into()));
            }
            ...
            if self.remaining() < 8 {
                return Err(CryptoError::Decryption("CBOR: truncated u64".into()));
            }
            let mut bytes = [0u8; 8];
            ...
            Err(CryptoError::Decryption(format!("CBOR: invalid additional info {}", additional)))
        }
    }

    fn read_uint(&mut self) -> Result<u64, CryptoError> {
        let byte = self.read_u8()?;
        let major = byte >> 5;
        let additional = byte & 0x1f;
        if major != 0 {
            return Err(CryptoError::Decryption(format!("CBOR: expected uint, got major type {}", major)));
        }
        self.read_argument(additional)
    }

    fn read_map_len(&mut self) -> Result<u64, CryptoError> {
        let byte = self.read_u8()?;
        let major = byte >> 5;
        let additional = byte & 0x1f;
        if major != 5 {
            return Err(CryptoError::Decryption(format!("CBOR: expected map, got major type {}", major)));
        }
        if additional == 31 {
            return Err(CryptoError::Decryption("CBOR: indefinite-length map not allowed".into()));
        }
        self.read_argument(additional)
    }

    fn read_bytes(&mut self) -> Result<&'a [u8], CryptoError> {
        let byte = self.read_u8()?;
        let major = byte >> 5;
        let additional = byte & 0x1f;
        if major != 2 {
            return Err(CryptoError::Decryption(format!("CBOR: expected bytes, got major type {}", major)));
        }
        let len = self.read_argument(additional)? as usize;
        if self.remaining() < len {
            return Err(CryptoError::Decryption("CBOR: truncated bytes".into()));
        }
    ...
    fn read_str(&mut self) -> Result<&'a str, CryptoError> {
        let byte = self.read_u8()?;
        let major = byte >> 5;
        let additional = byte & 0x1f;
        if major != 3 {
            return Err(CryptoError::Decryption(format!("CBOR: expected text, got major type {}", major)));
        }
        let len = self.read_argument(additional)? as usize;
        if self.remaining() < len {
            return Err(CryptoError::Decryption("CBOR: truncated text".into()));
        }
        let bytes = &self.data[self.pos..self.pos + len];
        self.pos += len;
        std::str::from_utf8(bytes).map_err(|_| CryptoError::Decryption("CBOR: invalid UTF-8 in text".into()))
    }

    fn skip_value(&mut self) -> Result<(), CryptoError> {
        let byte = self.read_u8()?;
        let major = byte >> 5;
        let additional = byte & 0x1f;
        ...
        match major {
            ...
            2 | 3 => {
                ...
                let len = self.read_argument(additional)? as usize;
                if self.remaining() < len {
                    return Err(CryptoError::Decryption("CBOR: truncated during skip".into()));
                }
                self.pos += len;
            }
            4 => {
            ...
            _ => {
                return Err(CryptoError::Decryption(format!("CBOR: unsupported major type {}", major)));
            }
        }
        Ok(())
    }
...
/// Decode header from CBOR bytes.
fn decode_header(data: &[u8]) -> Result<Sb2Header, CryptoError> {
    let mut r = CborReader::new(data);

    let map_len = r.read_map_len()?;
    if map_len > MAX_CBOR_KEYS as u64 {
        return Err(CryptoError::Decryption(format!("CBOR map has {} keys, max is {}", map_len, MAX_CBOR_KEYS)));
    }
    ...
    for _ in 0..map_len {
        ...
        match key {
            0 => {
                ...
                if bytes.len() != 32 {
                    return Err(CryptoError::Decryption(format!("context_id must be 32 bytes, got {}", bytes.len())));
                }
            ...
            3 => {
                let bytes = r.read_bytes()?;
                if bytes.len() != 16 {
                    return Err(CryptoError::Decryption(format!("inbox_kid must be 16 bytes, got {}", bytes.len())));
                }
            ...
            4 => {
                ...
                if s.len() > MAX_MSG_ID_LEN {
                    return Err(CryptoError::Decryption(format!("msg_id exceeds {} chars", MAX_MSG_ID_LEN)));
                }
                if !s.is_ascii() {
                    return Err(CryptoError::Decryption("msg_id must be ASCII".into()));
                }
            ...
            5 => {
                let bytes = r.read_bytes()?;
                if bytes.len() != 24 {
                    return Err(CryptoError::Decryption(format!("nonce must be 24 bytes, got {}", bytes.len())));
                }
            ...
            7 => {
                let bytes = r.read_bytes()?;
                if bytes.len() != 32 {
                    return Err(CryptoError::Decryption(format!("recipient_peerid must be 32 bytes, got {}", bytes.len())));
                }
            ...
            8 => {
                let bytes = r.read_bytes()?;
                if bytes.len() != 32 {
                    return Err(CryptoError::Decryption(format!("sender_ephemeral_pub must be 32 bytes, got {}", bytes.len())));
                }
            ...
            9 => {
                let bytes = r.read_bytes()?;
                if bytes.len() != 32 {
                    return Err(CryptoError::Decryption(format!("sender_peerid must be 32 bytes, got {}", bytes.len())));
                }
            ...
            10 => {
                let bytes = r.read_bytes()?;
                if bytes.len() != 64 {
                    return Err(CryptoError::Decryption(format!("sig must be 64 bytes, got {}", bytes.len())));
                }
                let mut sig = [0u8; 64];
            ...
            11 => {
                let bytes = r.read_bytes()?;
                if bytes.len() != 16 {
                    return Err(CryptoError::Decryption(format!("cert_id must be 16 bytes, got {}", bytes.len())));
                }
    ...
    if seen_required != REQUIRED_MASK {
        return Err(CryptoError::Decryption("Missing required header fields".into()));
    }
...
fn xchacha_decrypt(
    ...
    cipher
        ...
        .map_err(|_| CryptoError::Decryption("XChaCha20Poly1305 decryption failed".into()))
}

/// Parameters for SB2 encryption.
///
/// Groups all inputs to `Sb2::encrypt_params` to keep the API under
/// clippy's argument-count threshold while remaining explicit.
pub struct Sb2EncryptParams<'a> {
    /// Recipient's InboxKey X25519 public key (32 bytes)
    pub recipient_inbox_pk: &'a [u8; 32],
    /// Plaintext to encrypt (max 64 KiB)
    pub plaintext: &'a [u8],
    /// Thread identifier (32 random bytes)
    pub context_id: [u8; 32],
    /// Idempotency key (ASCII, max 128 chars)
    ...
    pub owner_peerid: &'a [u8; 32],
    /// Sender's Ed25519 public key (32 bytes)
    pub sender_peerid: &'a [u8; 32],
    /// Recipient's Ed25519 public key (32 bytes)
    pub recipient_peerid: &'a [u8; 32],
...
impl Sb2 {
    ...
    pub fn decode(data: &[u8]) -> Result<Self, CryptoError> {
        if data.len() < 6 {
            return Err(CryptoError::Decryption("SB2 too short".into()));
        }

        // Check magic
        if &data[..3] != SB2_MAGIC {
            return Err(CryptoError::Decryption("Invalid SB2 magic".into()));
        }

        // Check version
        let version = data[3];
        if version != SB2_VERSION {
            return Err(CryptoError::Decryption(format!("Unsupported SB2 version: {}", version)));
        }

        // Read header length (big-endian u16)
        let header_len = u16::from_be_bytes([data[4], data[5]]) as usize;
        if header_len > MAX_HEADER_LEN {
            return Err(CryptoError::Decryption(format!("SB2 header_len {} exceeds max {}", header_len, MAX_HEADER_LEN)));
        }

        if data.len() < 6 + header_len {
            return Err(CryptoError::Decryption("SB2 truncated: header incomplete".into()));
        }
    ...
    /// Encrypt plaintext to SB2 binary format using a params struct.
    ///
    /// For messages requiring sender authentication, call `sign()` after encryption.
    pub fn encrypt_params(params: Sb2EncryptParams<'_>) -> Result<Self, CryptoError> {
        ...
        if let Some(ref id) = params.msg_id {
            if id.len() > MAX_MSG_ID_LEN {
                return Err(CryptoError::Other(format!("msg_id exceeds {} chars", MAX_MSG_ID_LEN)));
            }
            if !id.is_ascii() {
                return Err(CryptoError::Other("msg_id must be ASCII".into()));
            }
        }
        ...
        let shared_secret = Zeroizing::new(x25519(*ephemeral_sk, *params.recipient_inbox_pk));

        // Derive symmetric key
        let key = derive_symmetric_key(&shared_secret, &ephemeral_pk, params.recipient_inbox_pk);
        ...
        // Compute inbox_kid
        let inbox_kid = Sb2Header::compute_inbox_kid(params.recipient_inbox_pk);
        ...
        // Build AAD
        let aad = build_aad(params.owner_peerid, params.canonical_path, &header_no_sig);

        // Encrypt
        let ciphertext = xchacha_encrypt(&key, &nonce, params.plaintext, &aad);
    ...
    ///
    /// Convenience wrapper around `encrypt_params`. For messages requiring
    /// sender authentication, call `sign()` after encryption.
    ...
    pub fn encrypt(
        ...
        Self::encrypt_params(Sb2EncryptParams {
            recipient_inbox_pk,
            plaintext,
            context_id,
            msg_id,
    ...
    ///
    /// Convenience wrapper around `encrypt_params`. Same as `encrypt`, but
    /// allows specifying a `cert_id` for AppKey-based signing.
    #[allow(clippy::too_many_arguments)]
    ...
    pub fn decrypt(
        ...
        if self.header.inbox_kid != expected_kid {
            return Err(CryptoError::Decryption("inbox_kid mismatch - key not found".into()));
        }
    ...
    pub fn verify_signature(
        ...
        let verifying_key = VerifyingKey::from_bytes(&self.header.sender_peerid)
            .map_err(|e| CryptoError::Decryption(format!("Invalid sender_peerid: {}", e)))?;

        let signature = Signature::from_bytes(sig_bytes);

        verifying_key
            .verify(&sig_input, &signature)
            .map_err(|_| CryptoError::Decryption("Signature verification failed".into()))?;
