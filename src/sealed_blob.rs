...
pub fn sealed_blob_encrypt_with_context(
    ...
    serde_json::to_string(&envelope).map_err(|e| CryptoError::Serde(e.to_string()))
}
...
pub fn sealed_blob_decrypt_with_context(
    ...
    let envelope: SealedBlobEnvelope = serde_json::from_str(envelope_json)
        .map_err(|_| CryptoError::Decryption(SealedBlobErrorCode::MalformedEnvelope.to_string()))?;
...
pub fn sealed_blob_verify_signature(
    ...
    let verifying_key = VerifyingKey::from_bytes(sender_ed25519_pk)
        .map_err(|_| CryptoError::Other("Invalid sender public key".to_string()))?;
