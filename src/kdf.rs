...
/// # Errors
///
/// Returns `CryptoError::KeyDerivation` if HKDF expansion fails (should never happen
/// with valid 32-byte output, but we handle it explicitly for robustness).
pub fn derive_noise_seed(ed25519_secret: &[u8; 32], device_id: &[u8]) -> Result<[u8; 32], CryptoError> {
